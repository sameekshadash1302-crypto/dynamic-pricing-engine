...
if st.sidebar.button("🧠 Get Pricing Recommendation"):
    ...
    st.pyplot(fig2)
tab1, tab2 = st.tabs(["🧠 Pricing Recommendation", "📊 Insights & Charts"])
...
current_price = st.sidebar.number_input(
    "Current Price (₹)", min_value=100, max_value=2000, value=800, step=10
)

competitor_price = st.sidebar.number_input(
    "Competitor Price (₹)", min_value=100, max_value=2000, value=780, step=10
)

promo = st.sidebar.selectbox("Promotion Active?", [0, 1], key="promo_select")


day_of_week = st.sidebar.slider(
    "Day of Week (0=Mon, 6=Sun)", 0, 6, 3, key="day_slider"
)


base_price = st.sidebar.number_input(
    "Base Price (₹)", min_value=100, max_value=2000, value=700, step=10
)
...
with tab1:
    # Button to compute scenarios (put computation & outputs inside this if)
    if st.button("🧠 Get Pricing Recommendation"):
        ...
        with col1:
            st.metric("Best Price (₹)", best_option[0])
        with col2:
            st.metric("Predicted Demand", best_option[1])
        with col3:
            st.metric("Revenue (₹)", best_option[2])
        ...
        # --- Charts: put inside the same if-block but inside tab2 context
        with tab2:
            import pandas as pd
            import matplotlib.pyplot as plt
            ...
            st.subheader("📈 Price vs Predicted Revenue Curve")
            fig1, ax1 = plt.subplots()
            ax1.plot(df_results["price"], df_results["revenue"], marker="o")
            ax1.set_xlabel("Price (₹)")
            ax1.set_ylabel("Predicted Revenue")
            ax1.set_title("Revenue Curve")
            st.pyplot(fig1)
            ...
            st.subheader("📉 Price vs Predicted Demand Curve")
            fig2, ax2 = plt.subplots()
            ax2.plot(df_results["price"], df_results["units"], marker="o")
            ax2.set_xlabel("Price (₹)")
            ax2.set_ylabel("Predicted Units Sold")
            ax2.set_title("Demand Curve")
            st.pyplot(fig2)
